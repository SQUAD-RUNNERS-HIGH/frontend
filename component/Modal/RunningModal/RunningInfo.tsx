  ...
  return (
    <View style={styles.infoContainer}>
      <View style={[styles.info]}>
        ...
        <Text style={[styles.key, isSmall && { fontSize: 10 }]}>경과시간</Text>
      </View>
      <View style={styles.info}>
        <Text style={[styles.value, isSmall && { fontSize: 14 }]}>
          {rest <= 0 && runningInfo.mode !== "solo" ? "러닝 종료" : `${rest}m`}
        </Text>
      ...
      <View style={styles.info}>
        <Text style={[styles.value, isSmall && { fontSize: 14 }]}>{speed}</Text>
        <Text style={[styles.key, isSmall && { fontSize: 10 }]}>속도</Text>
      </View>
    </View>
  );
};
